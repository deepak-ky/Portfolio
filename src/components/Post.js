    ...
    return (
        <main className='bg-green-100 min-h-screen p-12'>
            <section className='container mx-auto'>
                ...
                <div className='grid mid:grid-cols-2 lg:grid-cols-3 gap-8'>
                        ...
                        <article>
                            <Link to={"/post/" + postData.slug.current} key={postData.slug.current}>
                                <span className='block h-64 relative rounded shadow leading-snug bg-white border-l-8 border-green-400' key={index}>
                                    <img src={postData.mainImage.asset.url} alt={postData.mainImage.asset.alt} className="w-full h-full rounded object-cover absolute" />
                                    <span className='block relative h-full flex justify-end items-end pr-4 pb-4'>
                                        <h3 className='text-gray-800 text-lg font-blog px-3 py-4 bg-green-700 text-white bg-opacity-75 rounded'>{postData.title}</h3>
                                    </span>
                                </span>
                            </Link>
                        </article>
